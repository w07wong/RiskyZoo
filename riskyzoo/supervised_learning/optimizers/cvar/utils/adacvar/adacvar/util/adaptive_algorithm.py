...
from adacvar.adacvar.util.approximate_kdpp import ApproximateKDPP
from adacvar.adacvar.util.learning_rate_decay import Constant
    ...
    def __iter__(self):
        ...
        for _ in range(len(self)):
            ...
            if self._exp3.is_constant:
                ...
                # print(weights / np.sum(weights), self.num_indexes, self.set_size)
                kdpp_sample = np.random.choice(
                    self.num_indexes,
                    self.set_size,
                    # min(self.set_size, len(np.nonzero(weights))),
                    p=weights / np.sum(weights),
                    replace=False,
                )
