...
"""Generates training and testing data for label shift regression task. Returns tensors of data."""
class LabelShiftRegression():
    def generate_data(self, n_train, n_test, data_dim, seed=15, display=False, display_path=''):
        ...
        y_train = np.array([X_train[i].dot(theta) + eps_train[i] + 0.5 if X_train[i][0] <= 1.645 else X_train[i].dot(theta) + X_train[i][0] + eps_train[i] + 0.5 for i in range(n_train)])
